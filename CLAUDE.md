...
| `just check`     | build + clippy (matches workflows.json validationCommands) |
...
The spec validation commands (`cargo build` and `cargo clippy -- -D warnings`) are covered by `just check`.

## Codebase notes

- Rust edition 2024, toolchain 1.93.1.
- `#![allow(dead_code)]` in main.rs suppresses lint for scaffold stubs during early tasks; remove as modules are wired together.
- Specs live in `.ralph/specs/<name>/spec-source.md`; workflows live in `.ralph/workflows/<name>/workflows.json`. Store and Workflow in `src/ralph/` manage all access.
